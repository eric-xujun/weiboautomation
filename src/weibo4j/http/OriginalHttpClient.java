 ...
 * 
 */
public class OriginalHttpClient implements java.io.Serializable {
	...
	static Logger log = Logger.getLogger(OriginalHttpClient.class.getName());
	org.apache.commons.httpclient.HttpClient client = null;

	private MultiThreadedHttpConnectionManager connectionManager;
	private int maxSize;

	public OriginalHttpClient() {
		// change timeout to 2s avoid block thread-pool (Tim)
		this(150, 2000, 2000, 1024 * 1024);
	}

	public OriginalHttpClient(int maxConPerHost, int conTimeOutMs, int soTimeOutMs,
			int maxSize) {
	...
	public Response get(String url, PostParameter[] params)
		...
		if (null != params && params.length > 0) {
			String encodedParams = OriginalHttpClient.encodeParameters(params);
			if (-1 == url.indexOf("?")) {
				url += "?" + encodedParams;
			} else {
				url += "&" + encodedParams;
		...
		if (null != paging) {
			...
			if (null != params) {
				...
				if (0 != arrayPagingParams.length) {
					String encodedParams = OriginalHttpClient
