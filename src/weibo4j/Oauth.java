...
public class Oauth {
	...
	public AccessToken getAccessTokenByCode(String code) throws WeiboException {
		return new AccessToken(Weibo.oclient.post(
	...
	public String authorize(String response_type) throws WeiboException {
		String authorizeURL = WeiboConfig.getValue("authorizeURL").trim();
		String client_ID = WeiboConfig.getValue("client_ID").trim();
		String redirect_URI = WeiboConfig.getValue("redirect_URI").trim();
		return authorizeURL + "?client_id="	+ client_ID + "&redirect_uri=" + redirect_URI + "&response_type=" + response_type;
	}
}
